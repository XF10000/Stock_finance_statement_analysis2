...
策略：使用 FinancialDataManager 正确读取数据（自动解析JSON），逐个计算，批量写入
"""
import sqlite3
import pandas as pd
import time
...
def main():
    ...
    db_manager = FinancialDataManager(db_path)
    ...
    # 注意：不能一次性读取所有数据，因为数据是JSON格式，需要逐个解析
    print("\n步骤2：逐个读取并计算核心指标...")
    print("  （使用 FinancialDataManager 自动解析 JSON 数据）")
    ...
    for ts_code in tqdm(all_stocks, desc="计算进度"):
        try:
            # 使用 FinancialDataManager 读取数据（自动解析JSON）
            balance = db_manager.get_financial_data(ts_code, 'balancesheet')
            income = db_manager.get_financial_data(ts_code, 'income')
            cashflow = db_manager.get_financial_data(ts_code, 'cashflow')
    ...
    if len(all_indicators) > 0:
        indicators_df = pd.concat(all_indicators, ignore_index=True, sort=False)
    ...
    for _, row in indicators_df.iterrows():
        # 获取日期字段（支持中英文）
        end_date = row.get('end_date') if 'end_date' in row.index else row.get('报告期')
