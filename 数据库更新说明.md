...
本项目使用SQLite数据库（`database/market_data.db`）存储全A股财务数据和核心指标。数据库包含：
...
```bash
# 更新2024年年报数据
python update_market_data.py --update-latest --quarter 20241231

# 更新2025年一季报数据
python update_market_data.py --update-latest --quarter 20250331
```
...
```bash
# 如果只想更新原始数据，稍后再计算指标
python update_market_data.py --update-latest --no-indicators
```
...
# 4. 查看帮助
python update_market_data.py --help

# 5. 使用更多线程加速（默认5个）
python update_market_data.py --update-latest --workers 10
...
每季度财报发布后：
  ...
  - 10月底：更新当年Q2数据
    python update_market_data.py --update-latest --quarter 20250630
  
  - 次年1月：更新当年Q3数据
    python update_market_data.py --update-latest --quarter 20250930
```
...
**A**: 日志保存在 `update_market_data.log` 文件中：
```bash
tail -f update_market_data.log
```
...
# 或使用sqlite3导出
sqlite3 database/market_data.db .dump > backup_$(date +%Y%m%d).sql
```
...
```bash
# 从备份恢复
cp database/backup/market_data_20250316.db database/market_data.db

# 或从SQL文件恢复
sqlite3 database/market_data.db < backup_20250316.sql
```
